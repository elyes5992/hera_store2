...
const Navbar: React.FC = () => {
  ...
  const handleDrawerNavigation = (path: string) => { navigate(path); setMobileOpen(false); };
  ...
  // Auth Modal Handlers
  const handleOpenAuthModal = (mode: 'signIn' | 'signUp') => { setInitialAuthMode(mode); setAuthModalOpen(true); handleCloseUserMenu(); };
  const handleCloseAuthModal = () => setAuthModalOpen(false);

  // --- Drawer Content ---
  const drawer = ( /* ... Drawer content remains the same ... */
      <Box sx={{ width: drawerWidth }} role="presentation">
      <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", p: 1, background: theme.palette.mode === 'dark' ? theme.palette.grey[900] : 'linear-gradient(135deg, #FF8C00 0%, #FFA500 100%)', color: theme.palette.getContrastText(theme.palette.mode === 'dark' ? theme.palette.grey[900] : '#FF8C00'), }}> <Typography variant="h6" sx={{ my: 1, ml: 1, flexGrow: 1 }}> Hera Prints </Typography> <IconButton onClick={handleDrawerToggle} sx={{ color: "inherit" }}> <ChevronLeftIcon /> </IconButton> </Box>
      <Divider />
      <List onClick={handleDrawerToggle}>
        {navItems.map((item) => (<ListItem key={item.label} disablePadding> <ListItemButton sx={{ textAlign: "left", pl: 2 }} onClick={() => handleDrawerNavigation(item.path)} > <ListItemText primary={item.label} /> </ListItemButton> </ListItem> ))}
        <Divider sx={{ my: 1 }} />
        <ListItem disablePadding> <ListItemButton sx={{ textAlign: "left", pl: 2 }} onClick={() => { handleOpenAuthModal('signIn'); handleDrawerToggle(); }} > <ListItemText primary="Sign In / Sign Up" /> </ListItemButton> </ListItem>
  ...
  return (
    <>
      ...
      >
        <Toolbar sx={{ minHeight: { xs: "70px", sm: "90px" } }}>
          ...
          <Box sx={{ flex: 1, display: "flex", justifyContent: "flex-start", alignItems: "center" }}>
            <IconButton color="inherit" aria-label="open drawer" edge="start" onClick={handleDrawerToggle} sx={{ display: { md: "none" }, mr: 1 }} >
              <MenuIcon />
            </IconButton>
              ...
              {navItems.map((item) => (
                <Button
                  ...
                  sx={{
                    ...
                    fontWeight: 600,
                    position: "relative",
                    '&::after': { /* ... underline style ... */ content: '""', position: "absolute", bottom: "5px", left: "50%", width: "0%", height: "2px", backgroundColor: "white", transition: "all 0.3s", transform: "translateX(-50%)", boxShadow: '0 1px 2px rgba(0,0,0,0.3)' },
          ...
          <Box sx={{ display: "flex", alignItems: "center", cursor: "pointer" }} onClick={() => navigate("/")} >
            <Box sx={{
                ...
                padding: "1px", height: { xs: 40, sm: 44, md: 50 }, width: { xs: 40, sm: 44, md: 50 },
                overflow: "hidden",
                // --- Initial Floating Style ---
            ...
            <Typography variant="h4" component="div" sx={{
                display: { xs: "none", sm: "block" }, ml: 2, fontWeight: 600, letterSpacing: "0.5px",
                // --- Initial Floating Style ---
          ...
          <Box sx={{ flex: 1, display: "flex", justifyContent: "flex-end", alignItems: "center" }}>
            ...
            <Tooltip title="Account">
              <IconButton
                size="large" aria-label="account options"
                aria-controls={Boolean(anchorElUser) ? 'profile-menu-appbar' : undefined}
                aria-haspopup="true" onClick={handleOpenUserMenu}
                sx={{
                  color: "white", ml: 1,
                   // --- Initial Floating Style ---
                   transform: floatingTransform, // Apply slight lift
            ...
            <Menu
                id="profile-menu-appbar" anchorEl={anchorElUser}
                anchorOrigin={{ vertical: 'bottom', horizontal: 'right' }} keepMounted
                transformOrigin={{ vertical: 'top', horizontal: 'right' }}
                open={Boolean(anchorElUser)} onClose={handleCloseUserMenu} sx={{ mt: '5px' }}
            >
                <MenuItem onClick={() => handleOpenAuthModal('signIn')}> <Typography textAlign="center">Sign In</Typography> </MenuItem>
                <MenuItem onClick={() => handleOpenAuthModal('signUp')}> <Typography textAlign="center">Sign Up</Typography> </MenuItem>
            </Menu>
          </Box>
        </Toolbar>
      </AppBar>
      ...
      {/* --- Render AuthModal --- */}
      <AuthModal open={authModalOpen} handleClose={handleCloseAuthModal} initialMode={initialAuthMode} />
    </>
  );
};
